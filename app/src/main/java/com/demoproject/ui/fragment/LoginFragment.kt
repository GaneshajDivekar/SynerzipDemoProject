                        ...
                        .observe(viewLifecycleOwner, Observer {
                            if(it!=null)
                            {
                                ...
                                MyRoomDataBase.getInstance(activity!!).getNoteDao().deleteVisit()
                                    ...
                                    viewModel.getHomeScreen(it.data?.userId.toString(),it.data?.businessUnits?.get(0)?.businessUnitId.toString())
                                        .observe(viewLifecycleOwner,Observer{
                                            if(it!=null)
                                            {
                                                for (item in it.data!!.homescreenlist!!.indices) {
                                                    var homeScreenDB=HomeScreenDB()
                                                    homeScreenDB.app_menu_desc= it.data!!.homescreenlist?.get(item)?.appMenuDesc
                                                    homeScreenDB.app_menu_code=it.data!!.homescreenlist?.get(item)?.appMenuCode
                                                    homeScreenDB.app_menu_id=it.data!!.homescreenlist?.get(item)?.appMenuId.toString()
                                                    homeScreenDB.pending_item_count=it.data!!.homescreenlist?.get(item)?.pendingItemCount.toString()
                                                    MyRoomDataBase.getInstance(this!!.activity!!).getNoteDao()!!
                                                        .insertHome(homeScreenDB)
                                                }
                                                viewModel.getAllVisit(sessionManger.getUserID(),sessionManger.getCompanyId())
                                                    .observe(viewLifecycleOwner,Observer{
                                                        if(it!=null)
                                                        {
                                                            ...
                                                            {
                                                                var visitDB=VisitDB()
                                                                visitDB.visit_type_desc=it.data!!.get(i).visit_type_desc
                                                                visitDB.visitCategoryCode=it.data!!.get(i).visitCategoryCode
                                                                visitDB.bank_desc=it.data!!.get(i).bank_desc
                                                                visitDB.callStatus=it.data!!.get(i).callStatus
                                                                visitDB.siteAddress=it.data!!.get(i).siteAddress
                                                                visitDB.ticketNo=it.data!!.get(i).ticketNo
                                                                visitDB.online="0"
                                                                MyRoomDataBase.getInstance(activity!!).getNoteDao().insertVisit(visitDB)
                                                            }
